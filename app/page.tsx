...
export default function GalopeLibertador() {
  ...
  const LanguageButton = () => (
    <Button
      ...
      className="fixed top-4 left-4 z-50 bg-white/90 hover:bg-white text-black px-3 py-2 rounded-lg shadow-lg border border-gray-300 font-medium"
      size="sm"
    >
    ...
    >
      ...
      <div className="fixed top-4 right-4 z-50 flex items-center space-x-2">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="bg-white/90 hover:bg-white text-black p-1.3 rounded-full shadow-lg border border-gray-300 transition-all duration-200 hover:scale-105"
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        ...
        {/* Music Button moved to be part of the icon group */}
        <Button
          onClick={toggleMusic}
        ...
        <div className="min-h-screen flex items-center justify-center p-4">
          <div className="w-full max-w-6xl space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card className="bg-black/80 backdrop-blur-sm border-amber-600/50 p-6 space-y-4 animate-fade-in-up">
                <h3 className="text-2xl font-bold text-white mb-4">{t("howToPlay")}</h3>
                <div className="space-y-3 text-white">
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl">💻</span>
                    <span>
                      <strong>{t("desktop")}:</strong> {t("spaceToJump")}
                    </span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl">📱</span>
                    <span>
                      <strong>{t("mobile")}:</strong> {t("tapToJump")}
                    </span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl">⭐</span>
                    <span>{t("doubleJump")}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl">🧉</span>
                    <span>{t("collectItems")}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl">🌵</span>
                    <span>{t("avoidCacti")}</span>
                  </div>
                </div>
                <Button
                  onClick={startGame}
                  size="lg"
                  className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 text-xl"
                >
                  🐎 {t("play")}
                </Button>
              </Card>

              <Card className="bg-amber-50/90 backdrop-blur-sm border-amber-600/50 p-6 space-y-4 animate-fade-in-up">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-2xl font-bold text-amber-900">{t("highScores")}</h3>
                  <span className="text-3xl">🏆</span>
                </div>
                {highScores.length > 0 ? (
                  <div className="space-y-2">
                    {highScores.map((score, index) => (
                      <div
                        key={index}
                        className="flex justify-between items-center bg-white/60 rounded-lg p-3 border border-amber-200"
                      >
                        <span className="font-bold text-amber-900">#{index + 1}</span>
                        <span className="font-mono text-amber-800">{score.score}</span>
                        <span className="text-sm text-amber-700">{score.date}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-8">
                    <p className="text-amber-700 mb-4">{t("firstToPlay")}</p>
                    <p className="text-amber-600 text-sm">{t("newRecord")}</p>
                  </div>
                )}
              </Card>
            </div>
      ...
      {gameState === "playing" && (
        <div className="relative w-full h-screen">
          ...
          <div className="absolute top-4 left-4 z-10">
            <div className="bg-black/80 backdrop-blur-sm rounded-lg px-4 py-2 border border-amber-600/50">
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-10">
            <div className="bg-black/80 backdrop-blur-sm rounded-lg px-6 py-3 border border-amber-600/50">
              <p className="text-white text-center">
                <span className="font-bold">{t("controls")}:</span> {t("spaceOrTap")}
              </p>
            </div>
          </div>
        </div>
      ...
      {gameState === "gameOver" && (
        <div className="min-h-screen flex items-center justify-center p-4">
          <Card className="bg-black/90 backdrop-blur-sm border-red-600/50 p-8 text-center space-y-6 max-w-md w-full">
            ...
            <div className="space-y-2">
              ...
              {highScores.length > 0 && score >= highScores[0].score && (
                <p className="text-xl text-green-400 font-bold animate-pulse">{t("newHighScore")}</p>
              )}
            </div>
            <div className="space-y-3">
              <Button
                onClick={startGame}
                size="lg"
                className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 text-xl"
              >
                🔄 {t("playAgain")}
              </Button>
              <Button
                ...
                size="lg"
                variant="outline"
                className="w-full border-amber-600 text-amber-400 hover:bg-amber-600/20 font-bold py-3 text-xl bg-transparent"
              >
                🏠 {t("mainMenu")}
              </Button>
            </div>
